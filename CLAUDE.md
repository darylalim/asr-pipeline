...
`_fetch_youtube_audio` and `_fetch_url_audio` are `@st.cache_data(max_entries=5, ttl="1h")`. Both bounds matter, and for different reasons: these return **raw audio bytes** (up to `MAX_URL_DOWNLOAD_BYTES` = 500 MB each), so `max_entries` alone would let a long-running server sit on ~2.5 GB indefinitely — nothing evicts an entry that is never displaced. `ttl` is what actually releases the memory. Re-fetching an expired URL costs the user a download they already waited through once, which is the right trade against holding half a gigabyte per entry forever. `_transcribe` needs no `ttl` for the same reason it needs no size bound beyond `max_entries=20` — it caches result dicts (text + segments), not media.
...
Two accepted consequences: switching tabs now costs a server rerun, and `youtube_audio` / `url_audio` are `None` while another tab is active, so a pasted URL alone doesn't enable **Transcribe** unless its tab is showing. `key="input_tabs"` holds the active tab's *label* in session state; `AppTest` has no tab-selection API, so the tests drive tab switches by writing that key.
...
- Tab-gated fetches (`AppTest`) — a URL typed while another tab is active never calls `urlopen`; the same URL with the URL tab active fetches once and enables **Transcribe**. The active tab is set by writing `session_state["input_tabs"]` (AppTest can't click tabs), and `urlopen` is patched on `urllib.request`, not on `streamlit_app`, because AppTest re-executes the script each run and rebinds its `from urllib.request import urlopen`
