...
- **Release** (`.github/workflows/draft-release.yml`): a push to `main` touching `pyproject.toml` reads `project.version` and, unless `v<version>` is already tagged, opens a **draft** GitHub Release with auto-generated notes. Bumping `version` *is* the release action — an incidental bump in an unrelated `pyproject.toml` edit cuts a tag. Runs on plain `ubuntu-latest` (unlike CI) because it never calls `uv sync`, so the absent `mlx-whisper` Linux wheels don't matter

**Three failures land in CI with no local signal.** The `Stop` hook runs only `ty check` + `pytest`, so anything gated by `ruff` or by `uv sync --locked` passes every local check and fails on push:

- `uv.lock` pins the project's own version, so bumping `version` in `pyproject.toml` needs `uv lock` in the same commit — `uv sync --locked` fails otherwise
- `ruff format` covers **Markdown** (0.16 reformats Python inside fenced code blocks in `.md`), so CI's `ruff format --check .` gates `CLAUDE.md` and `README.md` — but the `PostToolUse` hook matches `*.py` only. Run `uv run ruff format .` by hand after editing a Python fence in either file
- selecting all of `E` enables `E501`, so an over-long line is a `ruff check` **error**, not something the formatter rewraps
