...
- **Never shoot below a ~700 px viewport** — `st.columns` stacks vertically there, so every `_labeled_toggle` row loses the side-by-side layout the whole UI is built on
...
- Every labelled control row shares one layout: `st.columns([3, 1], vertical_alignment="center")`, label in the left column via `st.markdown(label, help=...)`, control in the right with `label_visibility="collapsed"`. Build rows through `_labeled_toggle(label, help_text)` (label + a toggle right-aligned by an inner `st.container(horizontal_alignment="right")`, not by the column split itself) or `_field_label(label, help_text)` (label row only, for a widget that renders itself below it) — **never a bare `st.toggle`**, and help text hangs off the label markdown, not the widget's own `help=`. The Primary language selector hand-rolls the same `[3, 1]` split. Neither helper has test coverage, so a bare widget breaks the grid silently
